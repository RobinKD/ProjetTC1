...
def test_RF(trainX, trainy, validX, validy):
    ...
    for j in range(6, 20, 2):
        ...
        x.append(j)
        y.append(evaluation(validy, pred_valid))
    res.append([x, y, "Tuning Calibrated RF number max of features"])
    return res
...
def test_xgboost(trainX, trainy, validX, validy):
    ...
    for i in [1,2,3,4,6,8, 15, 30, 50, 100]:
        model = xgb.XGBClassifier(max_depth=i, objective='multi:softprob', n_jobs=3)
    ...
    x, y = [], []
    print("Tuning XGB number of estimators")
    for i in [1,2,3,4,6,8]:
        model = xgb.XGBClassifier(max_depth=5, n_estimators=100 * i, n_jobs=3, 
        ...
        x.append(i * 100)
        y.append(score)
    res.append([x, y, "Tuning XGB number of estimators"])
    x, y = [], []
    print("Tuning XGB learning rate")
    for i in [1, 3, 5, 10, 50, 100, 500]:
        model = xgb.XGBClassifier(max_depth=5, n_jobs=3,
    ...
    for i in [0,1,2,3,4,6,8]:
        model = xgb.XGBClassifier(max_depth=5, n_jobs=3,
                                  objective='multi:softprob', gamma=0.1 * i)
        model.fit(trainX, trainy, eval_metric=evaluation)
        pred_valid = model.predict_proba(validX)
        score = evaluation(validy, pred_valid)
        x.append(i * 0.1)
    ...
    for i in [1,2,3,4,6,8,10]:
        model = xgb.XGBClassifier(max_depth=5, n_jobs=100,
                                  objective='multi:softprob',
                                  min_child_weight=i)
        model.fit(trainX, trainy, eval_metric=evaluation)
        pred_valid = model.predict_proba(validX)
    ...
    for i in [1,2,3,4,6,8]:
        ...
        x.append(i * 0.1)
        y.append(score)
    res.append([x, y, "Tuning XGB subsample"])
    return res
...
def test_MLP(trainX, trainy, validX, validy):
    ...
    for i in [(100,60,20), (50,30,10), (100,50), (50,25), (100,), (75,),(50,), (30,), (20,)]:
        model = MLPClassifier(hidden_layer_sizes=i, max_iter=20)
        bagmodel = BaggingClassifier(model)
        bagmodel.fit(trainX, trainy)
        pred_valid = bagmodel.predict_proba(validX)
        x.append(str(i))
    ...
    for i in [j / 10 ** 6 for j in [1, 5, 10, 50, 100]]:
        model = MLPClassifier(max_iter=20, alpha=i)
        bagmodel = BaggingClassifier(model)
        bagmodel.fit(trainX, trainy)
        pred_valid = bagmodel.predict_proba(validX)
        x.append(i)
    ...
    for i in [j / 10**4 for j in [1, 5, 10, 50, 100]]:
        model = MLPClassifier(max_iter=20, learning_rate_init=i)
        bagmodel = BaggingClassifier(model)
        bagmodel.fit(trainX, trainy)
        pred_valid = bagmodel.predict_proba(validX)
        x.append(i)
    ...
    x, y = [], []
    print("Tuning activation function & solver")
    for i in ['identity', 'logistic', 'tanh', 'relu']:
        for j in ['lbfgs', 'sgd', 'adam']:
            model = MLPClassifier(max_iter=20, hidden_layer_sizes=(75), solver=j, activation=i)
            bagmodel = BaggingClassifier(model)
            bagmodel.fit(trainX, trainy)
            pred_valid = bagmodel.predict_proba(validX)
            x.append(str((i,j)))
            y.append(evaluation(validy, pred_valid))
    res.append([x, y, "Tuning activation function & solver"])
